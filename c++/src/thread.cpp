...
class Thread : public T
{
    ...
    Thread(bool detacted): createDetached_(detacted) {
        std::cout << "Thread inited!" << std::endl;
    };
...
void Thread<T>::spawnNewThread() {
    ...
    // Create thread
    if ((ret = pthread_create(&tid_, &attr, &threadStarter, static_cast<void*>(this))) != 0)
    {
        std::cerr << "Failed to set pthread stack size, error code:" << ret << std::endl;
    }
...
void Producer::run()
{
    std::cout << "this is producer thread:" << id << std::endl;
}
